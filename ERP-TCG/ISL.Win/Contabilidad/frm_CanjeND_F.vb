...
#Region "Variables"
    ...
    Private oe_Documento As e_MovimientoDocumento
    Private oe_DocumentoGen As e_MovimientoDocumento
    Private ol_Documento As l_MovimientoDocumento
    Private lo_Documento As List(Of e_MovimientoDocumento)
    ...
    Private oe_DetDocumento As e_DetalleDocumento
    Private ol_DetDocumento As l_DetalleDocumento
    Private lo_DetDocumento As List(Of e_DetalleDocumento)
...
#Region "Botones"
    ...
    Public Overrides Sub Exportar()
        Try
            If griListaNotaDespacho.Rows.Count = 0 Then Throw New Exception("No hay ningún dato para exportar al Excel")
            Exportar_Excel(griListaNotaDespacho, Me.Text)
            MyBase.Exportar()
        Catch ex As Exception
            MsgBox(ex.Message, MsgBoxStyle.Information, Me.Text)
        End Try
...
#Region "Metodos"
    ...
    Private Sub mt_InicializaLogicas()
        ol_Documento = New l_MovimientoDocumento
        ol_DetDocumento = New l_DetalleDocumento
    ...
    Private Sub mt_Listar()
        Try
            oe_Documento = New e_MovimientoDocumento
            oe_Documento.IdTipoDocumento = "GCH000000001"
            oe_Documento.TipoOperacion = ""
            oe_Documento.FechaInicio = dtp_FechaDesde.Value
            oe_Documento.FechaFinal = dtp_FechaHasta.Value
            lo_Documento = ol_Documento.Listar(oe_Documento)
            griListaNotaDespacho.DataSource = lo_Documento
            griListaNotaDespacho.DataBind()
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
...
#Region "Eventos"
    ...
    Private Sub ToolStripButton1_Click(sender As Object, e As EventArgs) Handles ToolStripButton1.Click
        Try
            If griListaNotaDespacho.Selected.Rows.Count > 0 Then
                If fc_ValidaCanje(griListaNotaDespacho.ActiveRow.Cells("Id").Value) Then
                    mt_MostrarDocumento(griListaNotaDespacho.ActiveRow.Cells("Id").Value)
...
#Region "Funciones"
    ...
    Private Sub mt_MostrarDocumento(IdDocumento As String)
        Try
            oe_Documento = New e_MovimientoDocumento
            oe_Documento.Id = IdDocumento
            oe_Documento = ol_Documento.Obtener(oe_Documento)

            oe_Venta = New e_Venta
            oe_Venta.IdMovimientoDocumento = IdDocumento
            oe_Venta = ol_Venta.ObtenerIdDocumento(oe_Venta)

            oe_DetDocumento = New e_DetalleDocumento
            oe_DetDocumento.TipoOperacion = "VEN"
            oe_DetDocumento.IdMovimientoDocumento = IdDocumento
            oe_DetDocumento.IndServicioMaterial = "M"
            lo_DetDocumento = ol_DetDocumento.Listar(oe_DetDocumento)
            ...
            gmt_ListarEmpresa("6", cbgCliente, oe_DocumentoGen.IdClienteProveedor, False)
            cbgCliente.Value = oe_DocumentoGen.IdClienteProveedor
            dtp_FechaEmision.Value = oe_Documento.FechaEmision
            Me.decTipoCambio.Value = gfc_TipoCambio(dtp_FechaEmision.Value, True)
            cboTipoPago.Value = oe_Venta.IdTipoPago
            cmbMoneda.Value = oe_DocumentoGen.IdMoneda
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
    ...
    Private Sub mt_LlenaObjeto()
        Try
            ...
            oe_DocumentoGen.TipoOperacion = "I"
            oe_DocumentoGen.IdSucursal = gs_PrefijoIdSucursal
            oe_DocumentoGen.PrefijoID = gs_PrefijoIdSucursal
            oe_DocumentoGen.Serie = txtSerie.Text
            oe_DocumentoGen.Numero = FormatoDocumento(CStr(gfc_ObtenerNumeroDoc(txtSerie.Text, cmbTipoDocumento.Value, 2)), 8)
            oe_DocumentoGen.IdEstadoDocumento = "1CH00014"
            oe_DocumentoGen.IdTipoDocumento = cmbTipoDocumento.Value
            oe_DocumentoGen.IdPeriodo = ""
            oe_DocumentoGen.IdTipoPago = cboTipoPago.Value
            oe_DocumentoGen.Mac_PC_Local = MacPCLocal()
            oe_DocumentoGen.IdClienteProveedor = cbgCliente.Value
            oe_DocumentoGen.Saldo = oe_DocumentoGen.Total
            oe_DocumentoGen.lstDetalleDocumento = New List(Of e_DetalleDocumento)
            oe_DocumentoGen.lstDetalleDocumento = lo_DetDocumento
            For Each detalle In oe_DocumentoGen.lstDetalleDocumento
                With detalle
                    .TipoOperacion = "I"
                    .Id = String.Empty
                ...
                End With
            Next
            oe_DocumentoGen.Venta = New e_Venta
            oe_DocumentoGen.Venta = fc_LlenarVenta()
            oe_DocumentoGen.IdUsuarioCrea = gUsuarioSGI.Id
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
    ...
    Private Sub mt_EmitirDocumento(IndMensaje As Boolean)
        Try
            ...
            oe_Cliente.Id = oe_DocumentoGen.IdClienteProveedor
            ...
            If frm.ShowDialog = Windows.Forms.DialogResult.OK Then
                ...
                oeDoc.Id = oe_DocumentoGen.Id : oeDoc.CargaCompleta = True

                oeDoc = ol_Documento.Obtener(oeDoc)
                ...
                If loAsientoModelo.Contains(oeAsientoModelo) Then
                    ...
                    If oeCuentaCorriente.Id <> "" Then
                        _banEmis = ol_Documento.GuardarVentaAsiento(oeDoc, oeAsientoModelo, oeServCtaCtble, True, String.Empty, False)
                    Else
                        ...
                        _banEmis = ol_Documento.GuardarVentaAsiento(oeDoc, oeAsientoModelo, oeServCtaCtble, False, String.Empty, False)
                    End If
    ...
    Private Function fc_LlenarVenta() As e_Venta
        Try
            ...
            With oe_Venta
                ...
                .Gravado = Math.Round(oe_DocumentoGen.SubTotal, 2)
                .IGV = Math.Round(oe_Documento.IGV, 2)
    ...
    Private Function fc_Guardar() As Boolean
        Try
            mt_LlenaObjeto()
            ol_Documento.GuardarCanje(oe_DocumentoGen, oe_Documento)
