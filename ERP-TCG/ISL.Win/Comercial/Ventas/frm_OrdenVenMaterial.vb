            ...
            If griOrdenComercialMaterial.Rows.Count > 0 Then
                Select Case e.Cell.Column.Key
                    ...
                    Case "IdAlmacen"
                        ...
                        '            oeAlmMaterial.IdEmpresaSis = gstrIdEmpresaSis
                        '            oeAlmMaterial.IdSucursal = gstrIdSucursal
...
#Region "Metodos"
    ...
    Private Sub mt_ControlBotoneria()
        Select Case ficOrdenComercial.SelectedTab.Index
            ...
            Case 1
                ...
                ElseIf oeOrdenComercial.Estado = "EVALUADO" And Operacion = "Editar" Then
                    gmt_ControlBoton(0, 0, 0, 1, 1)
                Else
    ...
    Private Sub mt_CargarCombos()
        Try
            ...
            oeCombo = New e_Combo
            oeCombo.IdEmpresaSis = gstrIdEmpresaSis
            ''Lugares
            ...
            'If _leEstAux2.Count > 0 Then ls_EstadoCuadrado = _leEstAux2(0).Id
    ...
    Private Sub mt_Mostrar()
        Try
            ...
            oeOrdDocumento.TipoExistencia = 1
    ...
    Private Sub mt_EjecutarOS()
        Try
            If griOrdenSalida.Selected.Rows.Count > 0 Then
                ...
                oeDetalleOrden.IdEmpresaSis = gstrIdEmpresaSis
                ...
                If olOrdenSalida.Guardar(oeOrdenSalida, New List(Of e_RegistroInventario)) Then
                    ...
                    'Dim OeOrdens As New e_Orden_Documento
                    'Dim olOrdens As New l_Orden_Documento
                    'Dim ListOrdens As New List(Of e_Orden_Documento)
                    'OeOrdens.TipoOperacion = ""
                    'OeOrdens.IdOrden = griOrdenSalida.ActiveRow.Cells("Id").Value
                    'ListOrdens = olOrdens.Listar(OeOrdens)
                    ' ''''''''''''''''''''''''''''''''''''
                    'Dim oeGuiaRR As New e_GuiaRR
                    'Dim olGuiaRR As New l_GuiaRR
                    'Dim lisGuiaRR As New List(Of e_GuiaRR)
                    'Dim oeGuiaRRDetalle As New e_GuiaRemisionRemitente_Detalle
                    'Dim olGuiaRRDetalle As New l_GuiaRemisionRemitente_Detalle
                    ''-----------------------------------------------------------------
                    'For Each row In ListOrdens
                    '    If row.TipoOrden = 1 Then
                    '        oeGuiaRR.Id = row.IdDocumento
                    '    End If
                    'Next
                    '' oeGuiaRR.Id = ListOrdens(1).IdDocumento
                    '' lisGuiaRR = olGuiaRR.Listar(oeGuiaRR)

                    'oeGuiaRR = olGuiaRR.Obtener(oeGuiaRR)

                    ' ''Detalle
                    '' oeGuiaRRDetalle.IdGuiaRemRemitente = ListOrdens(0).IdDocumento
                    'oeGuiaRRDetalle.IdGuiaRemRemitente = oeGuiaRR.Id
                    'oeGuiaRR.lstDetalleGuiaRemitente = olGuiaRRDetalle.Listar(oeGuiaRRDetalle)
                    ' ''''''
                    'oeGuiaRR.TipoOperacion = "O"
    ...
    Private Sub mt_ListarOS()
        Try
            ...
            oeOrdenSalida.TipoOperacion = "V"
            oeOrdenSalida.NroOrden = oeOrdenComercial.Id
            oeOrdenSalida.IdMovimientoInventario = 6
    ...
    Private Sub mt_ListarCtaCtble(ByVal numEjercicio As Integer)
        Try
            ...
            oeCtaCtble.TipoOperacion = ""
            oeCtaCtble.Movimiento = 1 : oeCtaCtble.CuentaAsociada = New List(Of e_CuentaAsociada) : oeCtaCtble.MonedaExtranjera = -1
            oeCtaCtble.FlujoCaja = -1 : oeCtaCtble.Conciliacion = -1
    ...
    Private Sub mt_EmitirDocumento(IndMensaje As Boolean)
        Try
            'Dim frm As New frm_EnviarDocumento("VEN")
    ...
    Private Function fc_GenerarOrdenSalida() As e_Orden
        Try
            ...
            With oeOrdenSalida
                ...
                .TipoOperacion = "I"
                .TipoReferencia = 6
                .Referencia = oeOrdenComercial.Id
                .TipoCambio = decTipoCambio.Value
